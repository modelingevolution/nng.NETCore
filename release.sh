...
# Functions
print_usage() {
    echo -e "${BLUE}nng.NETCore Release Script${NC}"
    echo
...
get_latest_version() {
    git tag --sort=-version:refname | grep -E '^nng/[0-9]+\.[0-9]+\.[0-9]+$' | head -n1 | sed 's/^nng\///' || echo "1.0.0"
}
...
print_info "Monitor progress at: https://github.com/modelingevolution/nng.NETCore/actions"
